...
async function companyNews(lead, companyIntel) {
  ...
  const { data, citations } = await groundedJSON(instructions, input, { contextSize: "medium" });
  ...
  return { items: sourced, citations };
}
...
async function personBrief(lead, companyIntel) {
  ...
  const { data, citations } = await groundedJSON(instructions, input, {
    contextSize: "medium",
    timeoutMs: 45000
  });
  if (!data || data.confirmed !== true) return { person: null, citations };
  ...
  if (!sources.length) return { person: null, citations };
...
async function domainScan(lead, companyIntel, thesis) {
  ...
  const { data, citations } = await groundedJSON(instructions, input, { contextSize: "high" });
  if (!data) return { domain: null, citations };
...
export async function buildDossier(lead, companyIntel = {}, thesis = null, { deadline = 0 } = {}) {
  ...
  if (deadline && Date.now() > deadline - 90000) return null; // no time to research properly
  ...
  if (!news.length && !person && (!domain || (!domain.competitors.length && !domain.domainMoves.length))) {
    return { empty: true, news: [], person: null, domain, krFit: [], hooks: [], summary: "", gaps: ["no sourced research returned"] };
  }
...
export function dossierMarkdown(lead, d) {
  ...
  if (d.empty) {
    L.push("**No sourced research returned.** Nothing below the sourcing bar was found for this prospect in the window. Do not send a generated email on this row without adding material by hand.");
    return L.join("\n");
  }
...
export function dossierSignal(d) {
  if (!d) return "";
  if (d.empty) return "dossier: empty (no sourced research)";
  const bits = [];
