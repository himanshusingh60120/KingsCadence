...
async function companyNews(lead, companyIntel, budget = 90000) {
  ...
  const { data, citations, error } = await groundedJSON(instructions, input, { contextSize: "medium", timeoutMs: budget });
...
async function personBrief(lead, companyIntel, budget = 45000) {
  ...
  const { data, citations, error } = await groundedJSON(instructions, input, { contextSize: "medium", timeoutMs: Math.min(45000, budget) });
...
async function domainScan(lead, companyIntel, thesis, budget = 90000) {
  const company = s(lead.companyName, 120);
  ...
  const { data, citations, error } = await groundedJSON(instructions, input, { contextSize: "high", timeoutMs: budget });
...
export async function buildDossier(lead, companyIntel = {}, thesis = null, { deadline = 0 } = {}) {
  ...
  // ── TIME BUDGET, MEASURED RATHER THAN GUESSED ────────────────────────
  // A search call can take up to 90s, plus a structuring call, plus the
  // fit/hook synthesis: roughly 110s for the whole dossier. The old guard
  // let it start with only 75s left, so on a slow row it ran past the
  // deadline and the four emails were all marked "out of time" — a row that
  // produced NOTHING because research overran. Skipping research entirely is
  // strictly better than that: you still get emails, just weaker ones.
  //
  // The per-call timeout is now derived from what is actually left, so a
  // tight budget shortens the searches instead of blowing the row.
  const remaining = deadline ? deadline - Date.now() : Infinity;
  const RESERVE_FOR_GENERATION = 110000;   // four emails, with retries
  if (remaining < RESERVE_FOR_GENERATION + 45000) return null;
  const searchBudget = Math.max(30000, Math.min(90000, remaining - RESERVE_FOR_GENERATION - 20000));
  ...
  const [newsRes, domainRes, personRes] = await Promise.all([
    cachedCompany ? Promise.resolve(cachedCompany.news) : companyNews(lead, companyIntel, searchBudget),
    cachedCompany ? Promise.resolve(cachedCompany.domain) : domainScan(lead, companyIntel, thesis, searchBudget),
    cachedPerson ? Promise.resolve(cachedPerson) : personBrief(lead, companyIntel, searchBudget)
  ]);
...
export function dossierMarkdown(lead, d) {
  ...
  if (d.empty) {
    if (d.error) {
      L.push(`**No usable research.** \`${d.error}\``);
      L.push("");
      // Blanket "check your API key" advice was wrong and wasted a run: the
      // key was fine and the search was running. Name the actual next step
      // for the actual failure.
      if (/WITHOUT searching/i.test(d.error)) {
        L.push("The model answered from memory instead of searching. This is a prompt or model problem, **not** a key problem. Run `npm run doctor` to see the live call, and try setting `RESEARCH_MODEL` to a model that supports the web search tool.");
      } else if (/no URLs at all|no sources or citations/i.test(d.error)) {
        L.push("The search ran but returned nothing verifiable. Run `npm run doctor`, which prints the raw prose and the evidence set, so this can be diagnosed instead of guessed at.");
      } else if (/OPENAI_API_KEY/i.test(d.error)) {
        L.push("The key is missing or malformed in this environment. Check `OPENAI_API_KEY` in Vercel, and remember it must be enabled for Production, Preview **and** Development.");
      } else {
        L.push("Run `npm run doctor` to reproduce this against the live API with full output.");
      }
      L.push("");
      L.push("This is a run-level fault: every row will look like this until it is fixed. It is not a fact about this prospect.");
    } else {
