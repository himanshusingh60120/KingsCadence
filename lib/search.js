...
export async function research(brief, question, {
  ...
  for (const model of MODEL_LADDER) {
    for (const rung of rungs) {
      ...
      if (!res) { lastError = `timeout after ${Math.round(timeoutMs / 1000)}s on ${model}`; continue; }
      if (!res.ok) {
        lastError = `${model}: ${await reasonOf(res)}`;
        // If the MODEL itself is the problem, the remaining rungs cannot help:
        // they only vary tool and parameters. Six wasted round trips per bad
        // model would eat the whole function budget before a good one is tried.
        if (res.status === 404 || /model/i.test(lastError) && /not (found|exist)|do not have access|unsupported model/i.test(lastError)) break;
        continue;
      }
