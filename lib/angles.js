...
export const ANGLES = [
  ...
  {
    key: "BLINDSPOT",
    name: "the question they cannot answer from inside",
    open:
      "Open on a QUESTION ABOUT THEIR OWN MARKET THAT THEIR INTERNAL DATA CANNOT ANSWER. This is the purest form of the pitch and it needs no news at all. " +
      "A company knows its own wins, its own pricing and its own pipeline. It does not know what it lost and why, what its rivals charged, which requirements it was scored against, or how buyers ranked the field. Those answers exist, they are just outside the building. " +
      "Name one such question precisely, in their vocabulary, about a segment they actually serve: who won the last set of recompetes in it and at what price, what the losing bids had in common, which requirement moved between rounds. " +
      "Then say plainly that this is the kind of question Kings Research answers, and what answering it would involve. The absence of a news hook is not a weakness here: the question is permanent and the gap is structural."
  },
  {
    key: "STRUCTURAL",
 ...
 * OPENING FORMS
 * ──────────────────────────────────────────────────────────────────────────
 * Ten emails came back with two opening shapes: seven began "Your ..." and
 * three began "New/Recent ...". The cause was mine. The opening guard listed
 * acceptable first words with "your" at the front, and a model handed a list
 * of permitted options takes the first one every time.
 *
 * The lesson, now three times over: any rule that permits a narrow set of
 * shapes becomes a template. Variety has to be ASSIGNED, not merely allowed.
 *
 * Each form specifies a grammatical shape for the first sentence. Assigned by
 * hash, so a given prospect always gets the same one and a list gets six.
 */
export const OPENING_FORMS = [
  { key: "QUESTION", rule: "Open with a direct question the reader cannot answer from inside their own company. Not rhetorical, not a hook: a real question whose answer would change something they do. The rest of the email is why you are asking and what it would take to answer it." },
  { key: "FACT_FIRST", rule: "Open with the hard fact itself, no preamble and no possessive. A number, a decision, a named change, stated flat. The reader's relevance to it comes in sentence two." },
  { key: "CONDITIONAL", rule: "Open with a conditional that puts the judgement in their hands: 'If X is already true in your book, then Y matters; if it is not, this is noise.' It respects that they know their business and you do not." },
  { key: "CONTRAST", rule: "Open by setting two things against each other: what the market assumes versus what the evidence shows, or what was true last year versus now. The tension is the hook." },
  { key: "SEGMENT", rule: "Open by naming a specific segment, customer type or programme of theirs, and stating what has moved inside it. Start with the segment noun, not with 'your'." },
  { key: "POSSESSIVE", rule: "Open on something of theirs directly: their renewals, their bids, their qualification window. This is the ONLY form permitted to begin with 'Your', and it is one form of six." }
];

export function chooseOpeningForm(lead = {}) {
  return OPENING_FORMS[hash(`open|${lead.email || ""}|${lead.companyName || ""}`) % OPENING_FORMS.length];
}
...
export const SUBJECT_SHAPES = [
  "name the prospect's company, then the outside move that touches them, joined by a plain word such as 'and' or 'after'",
  "name the prospect's company, then the specific thing of theirs now in question: a segment, a renewal cycle, a programme, a coverage class",
  "put the number or the named programme first, then the prospect's company, so the concrete detail lands before the name",
  "frame it as the question their own team is asking, with the prospect's company named inside it",
  "name the prospect's company and the regulation, contract vehicle or standard that just changed around them",
  "name the prospect's company and the segment where the movement is, with a qualifier that says what moved"
];


/**
 * LEAD-IN STANCES for the Email 1 bullet block.
 *
 * "We track your sector pricing quarterly. Here are three concrete insights:"
 * kept coming back on every row even after it was banned outright, because
 * the prompt asked for a lead-in and gave the model nowhere else to go. A ban
 * removes an option; it does not supply one. These do.
 *
 * Each is a STANCE, never a sentence, so nothing here can propagate verbatim.
 */
export const LEAD_INS = [
  "say where you would start if you were looking at this with them, framed as the first question you would want settled",
  "name what a team in their position usually needs to know before they can act on something like this",
  "offer it as the three things you would go and find out, if it were your call to make",
  "frame it as what the first month of work would answer, plainly, without naming a process or a package",
  "put it as what you would want on the table before their next planning or bid cycle",
  "state what you would check first, and why that order matters more than it looks"
];

export function chooseLeadIn(lead = {}) {
  return LEAD_INS[hash(`lead|${lead.email || ""}|${lead.companyName || ""}`) % LEAD_INS.length];
}
