...
export async function ensureOutputColumns(spreadsheetId, sheetName, headers) {
  ...
  // Sheets created before the Timezone column existed already have
  // "E1 Subject" onwards in place. Physically insert a new column right
  // before "E1 Subject" (so it lands as column X on the standard layout)
  // instead of appending it at the far end.
  if (!headers.includes("Timezone") && headers.includes("E1 Subject")) {
