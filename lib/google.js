...
// SHEET CONTRACT (order matters, it is what puts the block in I:N).
// With the standard 8 input columns in A:H, these land as:
//   I = E1 Subject   J = E1 Body   K = E2 Body   L = E3 Body   M = E4 Body
//   N = Status       O = Signal    P = Timezone
//
// ONE subject only. E2-E4 are thread follow-ups in Smartlead and inherit the
// E1 subject as "Re: ...", so generating three more subjects was wasted tokens
// and three more chances to trip the banned-word guard.
export const OUTPUT_COLUMNS = [
  "E1 Subject",
  "E1 Body", "E2 Body", "E3 Body", "E4 Body",
  "Status",
  "Signal",
  "Timezone",
  // The research behind the row, in the row. Not decoration: it is what lets
  // an operator tell a bad email caused by bad research from a bad email
  // caused by bad copy, which is the difference between fixing the retrieval
  // and fixing the prompt. It is also the thing a rep reads before a reply
  // lands and they have to hold a conversation.
  "Dossier",
  "Hooks"
];

// Legacy headers from the 4-subject era. Detected only so the app can warn
// that the sheet predates the I:N contract; never written to.
export const LEGACY_COLUMNS = ["E2 Subject", "E3 Subject", "E4 Subject"];
...
const HEADER_ALIASES = {
  ...
  // Optional: a real, specific data point / finding to lead the email with (the
  // "give"). When present, the email opens on it instead of a generic hook.
  insight: ["insight", "krinsight", "kingsinsight", "datapoint", "proofpoint", "hook", "finding"],
  // Optional: comma/semicolon-separated named competitors for this prospect.
  // When present these are searched for news too (a rival's move is the
  // strongest hook) and take precedence over model-derived competitor names.
  competitors: ["competitors", "competitor", "peers", "rivals", "similarcompanies", "competitorcompanies"]
};
...
// Read a tab that may not exist yet (the Insights library on a fresh sheet).
// Returns [] rather than throwing, so a missing library degrades the emails
// instead of breaking the run.
export async function readTabSafe(spreadsheetId, sheetName) {
  try {
...
export async function ensureOutputColumns(spreadsheetId, sheetName, headers) {
  ...
  // NOTE: the old "insert Timezone before E1 Subject" migration was removed.
  // It shifted every output column one to the right, which is exactly what
  // breaks the I:N contract Smartlead maps against. Output columns are now
  // only ever APPENDED, in OUTPUT_COLUMNS order, so I:N stays stable.
...
// Create `tabName` with `headerRow` if it does not exist yet. Returns true
// when the tab is present (pre-existing or just created).
...
// Append rows to a tab (creating it, with headerRow, when missing). Used for
// the "Derived Targets" prospecting tab: competitors of a prospect for whom a
// detected event is itself the outreach signal.
export async function appendRows(spreadsheetId, tabName, headerRow, rows) {
  if (!rows || !rows.length) return;
  const sheets = sheetsClient();
  await ensureTab(spreadsheetId, tabName, headerRow);
