...
export async function judgeRelevance(lead = {}, companyIntel = {}) {
  ...
  if (!title && !company) {
    return { relevant: false, verdict: "no", reason: "no title and no company on the row", source: "rule" };
  }

  // ── Fast path: obvious rejects, no model call ──────────────────────────
  for (const re of OBVIOUS_NO) {
    if (re.test(title)) {
      return { relevant: false, verdict: "no", reason: `role type is never a buyer (${(title.match(re) || [""])[0].trim()})`, source: "rule" };
    }
  }
  ...
  const user =
    ...
    `Return {"companyFit":true|false,"companyReason":"<8 words>","verdict":"buyer|influencer|no","reason":"<12 words, plain, why this seat would or would not engage>"}`;
  ...
  try {
    ...
    const res = gate.inJT
      ? { relevant: true, verdict: "influencer", reason: "relevance model unavailable, passed on title rules", source: "fallback" }
      : { relevant: false, verdict: "no", reason: `relevance model unavailable, ${gate.reason}`, source: "fallback" };
    cache.set(key, res);
    return res;
  }
  ...
  let res;
  if (!companyFit) {
    res = { relevant: false, verdict: "no", reason: `company does not buy market intelligence (${companyReason})`, source: "ai" };
  } else if (verdict === "no") {
    ...
    if (gate.inJT && String(gate.matched || "").startsWith("approved title")) {
      res = { relevant: true, verdict: "buyer", reason: `${gate.matched} (model was unsure)`, source: "rule" };
    } else {
      res = { relevant: false, verdict: "no", reason, source: "ai" };
    }
  } else {
    ...
    res = { relevant: true, verdict: finalVerdict, reason, source: "ai" };
  }
