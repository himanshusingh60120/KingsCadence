...
function buildContext(lead, companyIntel, news, personaKey, personaDesc, events, reportHook = null, insight = null, buyerWorld = "") {
...
export async function generateEmail(step, lead, companyIntel, news, events = [], usedSubjects = [], usedCTAs = [], insight = null, earlierBodies = [], buyerWorld = "") {
  const wantsSubject = step === 1; // E2-E4 are thread replies, no subject
  const [pKey, pDesc] = resolvePersona(lead);
  const ctx = buildContext(lead, companyIntel, news, pKey, pDesc, events, null, insight, buyerWorld);
