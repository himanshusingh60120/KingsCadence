...
export function subjectMagnet(subject, prospectCompany = "") {
  const sub = String(subject || "").trim();
  if (!sub) return "the subject is empty";
  if (sub.length > 52) return `the subject is ${sub.length} characters, over the 48 a phone shows. Cut it`;
  if (/:/.test(sub)) return "the subject uses a colon, which reads as a report title";

  const hasNumber = /\d/.test(sub);
  // Drop the first word: a sentence-initial capital is not a proper noun, and
  // counting it let "Cold-work flange-to-pipe connections" pass as if "Cold"
  // named something.
  const body = sub.split(/\s+/).slice(1).join(" ");
  const proper = (body.match(/\b[A-Z][a-zA-Z]{2,}/g) || []).concat(body.match(/\b[A-Z]{2,}\b/g) || []);
  const compTokens = String(prospectCompany || "").toLowerCase().split(/\s+/).filter((w) => w.length > 3);
  const mentionsProspect = compTokens.some((c) => sub.toLowerCase().includes(c));
  const outsideProper = proper.filter((p) => !compTokens.some((c) => p.toLowerCase().includes(c)));

  if (!hasNumber && !proper.length) {
    return "the subject names nothing: no company, no regulator, no programme, no number. Labels do not get opened";
  }
  // The killer case, and the most common one: the only thing the subject names
  // is the reader's own company or their own product. "Echodyne's new $40M
  // manufacturing hub" is Echodyne's own press release read back to them, and
  // the number does not rescue it.
  if (!outsideProper.length && mentionsProspect) {
    return `the subject is about ${prospectCompany}'s own news, which the reader already knows. Name the OUTSIDE move (a competitor, a regulator, a market shift) and the reader's stake in it`;
  }
  return null;
}
...
- "Dear ${first}," then a blank line.
- YOUR ENTRY ANGLE FOR THIS EMAIL IS FIXED, AND IT IS SPECIFIED IN THE ANGLE BLOCK ABOVE. Follow it. Do not default to opening with a competitor's name: that is one angle out of five and it is not yours unless the block says so.
- The opening must carry SUBSTANCE, not a headline restated. A fact plus a guess about its effect is not substance. The test: does the first half of this email tell the reader something about how their own world works that they had not connected? If it only tells them a thing happened and asserts it is bad for them, it is worthless and they have read forty of them this month.
- ONE line tying it to something concrete on their side: a cost, a bid, a renewal, a qualification, a budget cycle, a segment. Not "this will impact your market share", which asserts nothing.
...
SUBJECT: this is the whole email if it fails, so treat it as the hardest line to write.
- 3 to 7 words, UNDER 48 CHARACTERS. A phone shows about that much and nothing more.
- It must contain a real proper noun (a competitor, a regulator, a named programme) or a NUMBER. A subject with neither is a label, and labels do not get opened.
- The strongest shape connects THEIR world to the reader's stake: "Northrop's $3B and your renewals", "Wing's Florida move + your rates", "SeAH's US hub, your lead times". The reader should feel that something outside has moved and it touches them.
- NEVER make the subject about the prospect's own company or their own product. "Global Aerospace's in-orbit servicing technologies" and "Cold-work flange-to-pipe connections" describe things the reader already knows, and there is no reason to open.
- No colon, no title case, no report-title phrasing. Lowercase like a colleague typed it in a hurry.${common}`;
...
- Close with an open door and NO ASK: no CTA, no offer to send anything, no question mark. Write that final line yourself, differently from any stock closing. The absence of an ask is the entire point of this email. Do not put one back in.${common}`;
}
...
export async function generateEmail(step, lead, companyIntel, news, events = [], usedSubjects = [], usedCTAs = [], insight = null, earlierBodies = [], buyerWorld = "", thesis = null) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      out.body = ensureGreeting(cleanDashes(stripSignature(rawBody)), first);
      ...
      if (wantsSubject) {
        ...
        const magnet = subjectMagnet(out.subject, lead.companyName);
  ...
  if (last) {
    ...
    if (wantsSubject) {
      const stillBad = subjectMagnet(last.subject, lead.companyName) || subjectGeneric(last.subject) || subjectProblem(last.subject, comp, lead.title);
      if (stillBad) {
        try {
          const fix = await chatJSON(
            SYSTEM_PROMPT,
          ...
          if (fix.subject) {
            ...
            if (!subjectMagnet(s, lead.companyName) && !subjectGeneric(s) && !subjectProblem(s, comp, lead.title)) last.subject = s;
          }
