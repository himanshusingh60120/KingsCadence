...
const ENTITY_STOP = new Set((
  ...
  // Abstract nouns that routinely open a bullet or a sentence. None of these
  // is ever a company, and treating them as one blocked eight good rows.
  "insights insight analysis analyses assessment assessments evaluation review reviews " +
  "overview overviews mapping identifying understanding details detail breakdown breakdowns " +
  "competitive comparative quarterly monthly annual weekly daily specific detailed general " +
  "case cases study studies data trends trend perspectives perspective findings finding " +
  "who what where which why how recent new emerging key major leading current ongoing " +
  "market markets customer customers buyer buyers demand supply pricing price cost costs " +
  "growth share revenue budget budgets contract contracts bid bids proposal proposals " +
  "government agency agencies regulatory regulation regulations compliance procurement " +
  "technology technologies product products segment segments region regions industry " +
  "companies company firms firm teams team reply happy shall consider explore"
...
export function inventedEntityProblem(body, sourceText = "", lead = {}) {
  ...
  // SPLIT BY LINE FIRST, THEN BY SENTENCE.
  //
  // The previous version split on sentence punctuation alone, so a bullet
  // list read as one continuous sentence and the first word of every bullet
  // looked like a mid-sentence proper noun. That is why the last run blocked
  // rows for "organisations" called "Insights", "Analysis", "Competitive",
  // "Quarterly", "Assessment", "Detailed", "Specific" and "Case": every one
  // was simply the capitalised first word of a bullet.
  //
  // A bullet is its own unit and its first word is sentence-initial.
  const units = [];
  for (const line of b.split(/\n/)) {
    const cleaned = line.replace(/^\s*(?:[-*\u2022]|\d+[.)])\s*/, "");
    if (!cleaned.trim()) continue;
    for (const sentence of cleaned.split(/(?<=[.?!:])\s+/)) {
      if (sentence.trim()) units.push(sentence);
    }
  }
