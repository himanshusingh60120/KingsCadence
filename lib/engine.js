...
const ENTITY_STOP = new Set(("january february march april may june july august september october november december monday tuesday wednesday thursday friday saturday sunday kings research kings global q1 q2 q3 q4").split(" "));
...
export function inventedEntityProblem(body, sourceText = "", lead = {}) {
  ...
  for (const sentence of b.split(/(?<=[.?!:\n])\s+/)) {
    ...
    const flush = () => {
      ...
      // A single token still counts when it is CamelCase (GreenTech, DroneUp,
      // AeroVironment) or carries a corporate suffix, because those shapes are
      // never accidental capitalisation.
      else if (seq.length === 1 && (/[a-z][A-Z]/.test(seq[0]) || CORP_WORD.test(seq[0]))) candidates.add(seq[0]);
      run = [];
    };
    for (const w of words) {
      if (/^[A-Z][A-Za-z&.'’-]*$/.test(w) && w.length >= 2) run.push(w);
      else flush();
    }
    flush();
  }
  ...
  for (const cand of deduped) {
    ...
    if (ENTITY_STOP.has(lc)) continue;
...
export function linkProblem(body, step) {
  ...
  if (count === 0) return `the ${CONNECT_URL} link is missing. It belongs in the LAST sentence of the LAST paragraph, inside the sentence, never on a line of its own`;
  if (count > 1) return `the link appears ${count} times. Once only, in the closing sentence`;

  const paras = b.split(/\n\s*\n/).map((x) => x.trim()).filter(Boolean);
  const lastPara = paras[paras.length - 1] || "";
  if (!/kingsresearch\.com\/connect/i.test(lastPara)) {
    return "the link is buried in the middle of the email. Move it to the closing sentence of the final paragraph, so the reader finishes the thought before they are offered anywhere to go";
  }
  const sentences = lastPara.split(/(?<=[.?!])\s+/).filter(Boolean);
  const lastSentence = sentences[sentences.length - 1] || "";
  if (sentences.length > 1 && !/kingsresearch\.com\/connect/i.test(lastSentence)) {
    return "the link is not in the final sentence. It goes last, after the ask, so nothing follows it";
  }
  const line = lastPara.split("\n").find((l) => /kingsresearch\.com\/connect/i.test(l)) || "";
  const stripped = line.replace(/https?:\/\/\S+/g, "").replace(/[\s.,:;()\[\]-]/g, "");
  if (stripped.length < 12) {
    return "the link sits on its own line like a footer. Put it inside the closing sentence so it reads as part of the thought";
  }
  return null;
}
