...
const PITCH_SHAPES = [
  [/\bwe can help\b/i, 'a "we can help ..." capability menu. Replace it with ONE provenance line saying how you know the thing you just said'],
...
export function pitchProblem(body, step) {
  ...
  if (step === 1 || step === 2) {
    if (listItems >= 2) return "it contains a list. Emails 1 and 2 carry no list. Rewrite as plain sentences with a single provenance line";
  } else if (step === 3) {
...
- ONE PROVENANCE LINE, after the give and before the ask: how Kings Research knows this. Name the actual tracking or the actual work ("we track X quarterly", "we have been mapping Y since Z"). It must be specific to what you just said, not a description of the firm. If you cannot ground it in the material above, write "This came out of our ${sector} tracking" and no more.
...
- NO bulleted list, NO "we can help ${comp} with:", NO paragraph about what the firm does. One provenance line is the entire self-introduction Email 1 gets.
...
export async function generateEmail(step, lead, companyIntel, news, events = [], usedSubjects = [], usedCTAs = [], insight = null, earlierBodies = []) {
