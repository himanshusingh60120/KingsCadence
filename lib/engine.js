...
- ANCHOR PRIORITY for Email 1, never inverted: (1) a supplied real insight, (2) a relevant competitor or market/regulatory move and its implication for the reader, (3) the reader's own event with an OUTSIDE angle, (4) a grounded point about their business.
...
- CTA: a DIFFERENT reply/value ask from E1, offer to send something specific. No meeting, no "20 minutes".${common}`;
...
- CTA: offer to send that named read, closing line different from E1 and E2.${common}`;
...
- OPEN DOOR: warm, low-pressure, reply-based, offer to send your read if it is useful. No meeting, no "20 minutes", no "12 to 24 months".${common}`;
}
...
export async function generateEmail(step, lead, companyIntel, news, events = [], usedSubjects = [], usedCTAs = []) {
  ...
  if (last) {
    if (!wantsSubject) return { subject: "", body: last.body };
    const stillBad = subjectGeneric(last.subject) || subjectProblem(last.subject, comp, lead.title);
    if (stillBad) {
      try {
        const fix = await chatJSON(
          SYSTEM_PROMPT,
          `The email body below is final. Write ONLY a new subject line for it: 3 to 7 words, hyper-specific to this prospect (name the competitor move, event, product, segment, or company in the body), conversational, none of the banned subject words, in the reader's frame (never we/our/us). The previous subject "${last.subject}" was rejected because: ${stillBad}. Return ONLY {"subject":"..."}.\n\nBODY:\n${last.body}`
        );
        if (fix.subject) {
