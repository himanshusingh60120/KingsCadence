...
import { chooseAngle, chooseClosing, chooseSubjectShape, chooseLeadIn } from "./angles";
...
const BANNED_PATTERNS = [
  [/\bwe\s+(?:track|monitor|follow|watch)\b[^.!?]{0,60}\b(?:quarterly|weekly|monthly|daily|continuously|closely|constantly|regularly)\b/i,
...
function numericClaims(text) {
  const out = [];
  const t = String(text || "");
  // Percentages, currency amounts, and multipliers: the shapes a reader checks.
  for (const re of [
    /\b\d{1,3}(?:\.\d+)?\s?%/g,
    /[$£€]\s?\d[\d,.]*\s?(?:billion|million|bn|m|k)?/gi,
    /\b\d[\d,.]*\s+(?:billion|million|thousand)\b/gi,
    /\b\d+x\b/gi
  ]) for (const m of t.matchAll(re)) out.push(m[0].trim());
  return [...new Set(out)];
}

/** Digits only, so "$3 billion" matches "3 billion" or "3B" in a headline. */
...
export function fabricationProblem(body, sourceText = "") {
  ...
  if (src.length < 40) {
    // No corpus to check against, so ANY figure is unverifiable.
    const any = numericClaims(b);
    if (any.length) {
      return `it states figures (${any.slice(0, 3).join(", ")}) with no research behind them on this row. Remove every number and write only what can be supported, or the email is a liability`;
    }
    return null;
  }

  // Exact digit-string match only. A looser check let "$2 billion" pass by
  // matching the "2" inside a publication date of 2026, which is precisely the
  // kind of near-miss this guard exists to catch.
  const srcDigits = new Set([
    ...numericClaims(src).map(digitsOf),
    ...(src.match(/\b\d[\d,.]*/g) || []).map(digitsOf)
  ].filter(Boolean));

  const unsupported = numericClaims(b).filter((c) => {
    const d = digitsOf(c);
    if (!d) return false;
    return !srcDigits.has(d);
  });
...
function buildContext(lead, companyIntel, news, personaKey, personaDesc, events, reportHook = null, insight = null, buyerWorld = "", thesis = null, angle = null, subjectShape = "", closing = "", leadIn = "") {
...
export async function generateEmail(step, lead, companyIntel, news, events = [], usedSubjects = [], usedCTAs = [], insight = null, earlierBodies = [], buyerWorld = "", thesis = null, deadline = 0) {
  ...
  const sourceCorpus = [
    ...
    (companyIntel && companyIntel.description) || "",
    (companyIntel && companyIntel.siteText) || "",
    thesis ? [thesis.whatTheyAre, ...(thesis.segments || []), ...(thesis.deliverables || [])].join(" ") : ""
  ].filter(Boolean).join(" \n ");
  ...
  const ctx = buildContext(lead, companyIntel, news, pKey, pDesc, events, null, insight, buyerWorld, thesis, angle, subjectShape, closing, leadIn);
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      const fabIssue = fabricationProblem(out.body, sourceCorpus);
  ...
  if (last) {
    ...
    const bodyIssues = () => [
      ...
      fabricationProblem(last.body, sourceCorpus),
    ...
    if (stuck.length && (!deadline || Date.now() < deadline - 15000)) {
      try {
        ...
        if (fix.body) {
          const cleaned = normalizeProse(ensureGreeting(cleanDashes(stripSignature(fix.body)), first));
