...
export function fabricationProblem(body, sourceText = "") {
  ...
  const srcDigits = new Set([
    ...
    // Bare numbers count, EXCEPT four-digit years: a 2026 publication date is
    // not evidence for a "$2 billion" claim, and treating it as such is how
    // fabricated figures were passing.
    ...(src.match(/\b\d[\d,.]*/g) || [])
      .filter((n) => !/^(19|20)\d{2}$/.test(n.replace(/[,.]/g, "")))
      .map(digitsOf)
  ].filter(Boolean));
...
function buildContext(lead, companyIntel, news, personaKey, personaDesc, events, reportHook = null, insight = null, buyerWorld = "", thesis = null, angle = null, subjectShape = "", closing = "", leadIn = "", openingForm = null) {
...
export async function generateEmail(step, lead, companyIntel, news, events = [], usedSubjects = [], usedCTAs = [], insight = null, earlierBodies = [], buyerWorld = "", thesis = null, deadline = 0) {
  ...
  const ctx = buildContext(lead, companyIntel, news, pKey, pDesc, events, null, insight, buyerWorld, thesis, angle, subjectShape, closing, leadIn, openingForm);
