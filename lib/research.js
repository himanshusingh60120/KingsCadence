 ...
 * prospect. That angle is what EMAIL 1 opens on. Discards stock-price noise,
 * listicles, and same-name-different-company stories. Returns [] on no signal
 * so the engine cleanly falls back to a sector-level opener.
 */
export async function classifyEvents(companyName, industry, news) {
  ...
  const system =
    ...
    "question it forces on the company's leadership. You are ruthless about noise: " +
    "you discard stock-price moves, analyst ratings, listicles/round-ups, generic " +
    "market-size stories, and any headline that is clearly about a different company " +
    "with a similar name. A false event is worse than none. Return ONLY valid JSON.";
  ...
  const user =
    ...
    `{"events":[{"type":"<one event type above>","scope":"company|industry",` +
    `"recency":"<date if known, else empty>","what":"<one-sentence factual summary of the event>",` +
    `"angle":"<the specific decision, cost, or risk this forces on the prospect, phrased as an advisor would raise it>"}]}\n\n` +
    `Rules: keep at most 4 events, strongest first, prefer company-scope over industry-scope. ` +
    `If NO genuine event is present, return {"events":[]}. Never invent an event that is not supported by a headline.`;

  try {
    const out = await chatJSON(system, user, { temperature: 0.4, maxTokens: 700 });
