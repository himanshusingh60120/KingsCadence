...
export async function newsSignals(companyName, industry, opts = {}) {
  ...
  const QUOTA = { competitor: 6, company: 4, industry: 2, "own-site": 2 };
  const prioritized = [];
  for (const scope of ["competitor", "company", "industry", "own-site"]) {
    prioritized.push(
 ...
 */
export async function classifyEvents(companyName, industry, news, companyDesc = "", competitors = []) {
