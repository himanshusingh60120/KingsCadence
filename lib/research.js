...
export function quarterWindowDays(now = new Date()) {
  ...
  return daysIn < 28 ? daysIn + 56 : daysIn;
}
...
export async function newsSignals(companyName, industry, opts = {}) {
  const { domain, headCount, revenue, subIndustry, competitors, domainQueries } = opts;
  const items = [];
  const queries = [];
  ...
  for (const dq of (domainQueries || []).slice(0, 5)) {
...
export async function classifyEvents(companyName, industry, news, companyDesc = "", competitors = [], marketCtx = null) {
  ...
  try {
    ...
    return events
      ...
      // NO CONSEQUENCE, NO EVENT. An event the classifier cannot tie to a
      // concrete change for this company is exactly what forced the model to
      // hedge ("this may influence...", "could reshape..."). Dropping them at
      // source is what stops the hedging, not banning the words.
      .filter((e) => e.subject === "self" || (e.affects && e.affects.length > 8))
